...
\newcommand\arclabel[6][stealth-stealth,shorten <=1pt,shorten >=1pt]{
    \chemmove{
        ...
        \node[shift=(\anglestart:#2+1pt)#4,anchor=\anglestart+180,rotate=\anglestart-90,
        inner sep=0pt,outer sep=-8.5pt]at(#4){#6};
    }
}

% originale
\newcommand\arclabeltwo[6][stealth-stealth,shorten <=1pt,shorten >=1pt]{
    \chemmove{
        \arcbetweennodes{#4}{#3}\anglestart \arcbetweennodes{#4}{#5}\angleend
        \draw[#1]([shift=(\anglestart:#2)]#4)arc(\anglestart:\angleend:#2);
        \pgfmathparse{(\anglestart+\angleend)/2}\let\anglestart\pgfmathresult
        \node[shift=(\anglestart:#2+1pt)#4,anchor=\anglestart+180,rotate=\anglestart+90,
        inner sep=0pt,outer sep=0pt]at(#4){#6};
    }
}
...
\begin{figure*}[h!]
\centering \includegraphics[width=.8\textwidth]{grafico legame covalente.png}
\end{figure*}\\
...
\fbox{
    \makecell[l]{ 
        \hspace*{.5cm}Geometria \hspace*{1.65cm} Prospettiva \hspace*{1.05cm} Modello a sfere \\ \\
        Geometria lineare: \qquad
        \chemfig{\Charge{90=\|,-90=\|}{O}=C=\Charge{90=\|,-90=\|}{O}} \qquad
        \chemfig{@{1}\circled{O}=@{2}\circled{C}=@{3}\circled{O}}
        \arclabel{-0.5cm}{1}{2}{3}{\footnotesize180\textdegree} \\ \\ \\
        Triangolare planare: \hspace*{.175cm}
        \chemleft[
            \chemfig{
                {\Charge{90=\|,-180=\|}{O}}
        ...
        \chemright]$^{2-}$ \hspace*{.35cm}
        \chemfig{
            \circled{O}
            =[:-45]@{2}\circled{C}(-[:-90]@{1}\circled{O})
            -[:45]@{3}\circled{O}
            \arclabeltwo{0.5cm}{1}{2}{3}{\footnotesize120\textdegree}
        }
    }
}
