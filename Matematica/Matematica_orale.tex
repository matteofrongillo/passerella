...
\sdefinition{Limite di una funzione reale}{
    Data una funzione \(f:D\to \mathbb{R}\) e un punto un punto \(c\),
    il limte \[L=\lim_{x\to c}f(x)\] con \(x\) tendente a \(c\) esiste se dato un
...
\[
    ...
    |f(x)-L|<\epsilon
\]
Che significa: Quando ci troviamo nella striscia rossa (\(|x-c|<\delta\)),
...
\sdefinition{Equivalenza asintotica}{
...
Una volta stabilito che \(f(x)\) è asintoticamente equivalente ad una retta con pendenza \(m\),
...
solo da una parte, come per esempio la funzione \(y=\ln(x)\). Oppure, la funzione potrebbe anche tendere
a qualsiasi altro numero finito come \(10\) o \(e^\pi\).
...
\sdefinition{Continuità di una funzione reale}{
    Data una funzione reale \(f\colon D \to C\), la funzione è continua nel punto \(x=c\) dove \(c\in D\) se
    \[
        \lim_{x\to c} f(x) = f(c)
    \]
    e quindi se il limite esiste ed è uguale al valore della funzione in quel punto.
}
...
\begin{itemize}
    \item Si dice che una funzione è continua in un punto se il limite in quel punto esiste ed è uguale al valore della funzione nel punto;
    \item si dice che una funzione è continua in un sottointervallo del dominio se è continua in tutti i punti di suddetto intervallo;
    \item si dice che una funzione è continua se è continua in tutti i punti nel suo intervallo. 
\end{itemize}
...
\begin{enumerate}
    ...
    \item La funzione \(f(x)=\frac{1}{x}\) è continua nonostante \(f(0)=\text{indeterminato}\).\\
        Ciò è dato dal fatto che la funzione è continua su tutto il piano sinistro, ed è anche continua su tutto il piano destro.
        Nonostante ci sia un distacco fra i valori negativi e positivi, il punto \(x=0\) non è compreso nel dominio.
        Di conseguenza, anche se la funzione è costituita da due parti staccate, è comunque continua in quanto è continua su tutti i punti \underline{del dominio}.
\end{enumerate}
...
\subsection{Significato geometrico della derivata}
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        Questa pendenza è rappresentata dalla retta tangente.
        \[
...
della posizione \(x(t)\), e l'accelerazione \(a(t)\) rappresenta in ogni punto il cambiamento
...
Assumiamo quindi che \(f'(a)\) esista e non sia \(\pm \infty\):
\begin{enumerate}
...
\textbf{\color{red}Importante:} La tangente potrebbe essere verticale, in tal caso ha una funzione del tipo \(x=a\).
...
\stheorem{Teorema di Bernoulli-de l'Hôpital}{
    ...
    in un intorno di \(x_0\in\mathbb{R}\) (non necessariamente in \(x_0\)).
...
La monotonia della funzione centra con il suo comportamento di crescita o di decrescita.
...
\end{itemize}
Queste proprietà sono possono valere sia per tutto il dominio della funzione che solo su un sottointervallo.
...
\begin{itemize}
    ...
    \item Se \(f'(a) = 0\), allora \(f(x)\) è un punto critico in \(x = a\). Questo è un punto in cui la funzione va in stallo, e quindi
        potrebbe trattarsi di un punto di massimo, di minimo, di flesso, o semplicemente un punto dove la funzione è piana.
    \item Se \(f'(a)\) non è definita, allora \(f(x)\) è un punto critico in \(x = a\).
    Questo punto potrebbe presentare varie geometrie, come punti angolosi, punti cuspidali, o altre geometrie.
    In generale, dovete immaginarvi che la derivata non esista (il limite geometrico non esiste), e quindi esistono infinite casistiche.
    Magari, dalla parte sinistra la funzione è totalmente normale e tende a un valore finito, mentre dall'alto la funzione tende a \(-\infty\) (TODO: mettere alcune illustrazioni
    della spettacolare varietà di questi tipi di punti).
\end{itemize}
...
\begin{minipage}{0.5\textwidth}
    \begin{tikzpicture}[
        ...
        \node[right] at (0, Height) {\footnotesize Verso l'alto, decrescente};
        \node[right] at ({Width / 2}, Height) {\footnotesize Verso l'alto, crescente};
        \node[right] at (0, Height / 2 - 0.15) {\footnotesize Verso il basso, decrescente};
        \node[right] at ({Width / 2}, {Height / 2 - 0.15}) {\footnotesize Verso il basso, crescente};
...
\begin{minipage}{0.5\textwidth}
    \phantom{ } \\
    \begin{itemize}
        \item \(f(x)\) è \textbf{concava verso l'alto} in un intervallo \(I\) se tutte le tangenti in \(I\) sono soto il grafico.
        \item \(f(x)\) è \textbf{concava verso il basso} in un intervallo \(I\) se tutte le tangenti in \(I\) sono sopra il grafico.
    \end{itemize}
    \hphantom{ } \\
    \begin{itemize}
    ...
    Questo funzione perché quando la funzione è concava in sù, cresce o decresce sempre di più. Per cui \(f'(x)\) 
...
\sdefinition{Punto di flesso}{
    Un punto di flesso è un punto in cui la funzione è continua e la concavità della funzione cambia, ossia dove la
    funzione passa da concava verso l'alto a concava verso il basso o viceversa. Inoltre, la funzione deve essere derivabile in quel punto.
}
...
In fisica, quando abbiamo la funzione dell'accelerazione \(a(t)\), possiamo trovare la funzione della velocità
...
%PART PROVVISORIA, CONTROLLARE TUTTO!!
\newpage
\part{Stocastica}
...
\begin{figure}[ht!]
    \begin{center}
            \begin{tikzpicture}[scale=1.75]
                ...
                \node[overlay] at (-0.2,2.75) {\(\mathbf{\Omega}\)};
            \end{tikzpicture}
    \end{center}
    \end{figure}
...
\figbox{
    \begin{tikzpicture}[scale=2]
        ...
        \node[overlay] at (0.2,3.75) {\(\mathbf{\Omega}\)};
    \end{tikzpicture}
}
